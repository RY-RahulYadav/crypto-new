...
const SelectAssetSwap = () => {
  const navigate = useNavigate();
  const [isWalletModalOpen, setIsWalletModalOpen] = useState(false);
  const [isCryptoFromModalOpen, setIsCryptoFromModalOpen] = useState(false);
  const [isCryptoToModalOpen, setIsCryptoToModalOpen] = useState(false);
  const [selectedWallet, setSelectedWallet] = useState({
    name: 'My Main Wallet',
    balance: '0.1234 BTC'
  });
  const [selectedCryptoFrom, setSelectedCryptoFrom] = useState('BTC');
  const [selectedCryptoTo, setSelectedCryptoTo] = useState('ETH');
  ...
  const handleCryptoFromSelect = (crypto) => {
    setSelectedCryptoFrom(crypto);
    setIsCryptoFromModalOpen(false);
    // If the selected "to" crypto is the same as the new "from" crypto, reset "to"
    if (selectedCryptoTo === crypto) {
      const availableCryptos = cryptos.filter(c => c.symbol !== crypto);
      if (availableCryptos.length > 0) {
        setSelectedCryptoTo(availableCryptos[0].symbol);
      }
    }
  };

  const handleCryptoToSelect = (crypto) => {
    setSelectedCryptoTo(crypto);
    setIsCryptoToModalOpen(false);
  };
  ...
  const cryptos = [
    { name: 'Bitcoin', symbol: 'BTC', logo: 'https://lh3.googleusercontent.com/aida-public/AB6AXuBsPu3o7Zz0DYncl7Mxq2HlgFhzu1D81sIgtu-L5xnuO2QW8KXGqb7fpg02kge2rNgZyPx9y-zsgu2CMfDEpR1m49FQo07673nfd1SzR7KtHGVaYVVI7ul3HNvjbZidPS42ZoII7J1wD63eBgONeCsnVy8Wlj4gOeEdcdkLomItY9CaIPh4jFOEHdcIkJLls_hxHXKK-kWT4mXDDxuPASPpsNUUfgK5UNDvfAGGX0o_y_tZIeWEDOIuTRyD9rfCc7jzG1jMjBc3pgU' },
    { name: 'Ethereum', symbol: 'ETH', logo: 'https://lh3.googleusercontent.com/aida-public/AB6AXuCCpuY9TKv2o5cWqRmXTMo-ekPWNQim6ydGNckJKjmm9Glq23KT_8R-p-OhxHaK6BylNkR4ByLullLlaSVywyuDpioOx7OeZ_06-MXnPP2JSRz-J6knkUv2n-TSuMD8ZxwSXTqVynJswZxPQt6DapQzbA2jzO0a62LdlASvUuzoHzjKeIPQII_JCEx3m1yB-EkWNj1iBjf82uDwh2_iYaqNpAz4rsspaIJ06SplXqn9wVoiooi1z8Y5RwTD3mMBmQaijCbOrroXjbQ' },
    { name: 'Solana', symbol: 'SOL', logo: 'https://lh3.googleusercontent.com/aida-public/AB6AXuAv3XDIhjkQlP2x3gBeqm1WD-hS-dJjVsCIbuoaPnX6UBaQAuFqRMtVpO6WVrpSeDdv9UURJoeXLtEllXYROe_ilmGIctG3qA-Xk_99NYuHgdd2K6eev1b-65CqiVb8_hirMM04-PQ_R0UMGQ_sGAlcBkXfQeGQVp1pQB9llnZnvnQgbqb5lLch4aud1K9zfrfiW7ByqV50vz08mnn29iXhfjpr6gt1YrzeD43cHCWr_LQpWEYYzwtljXQ5B0qYDReglGE8ffXiOUU' },
    { name: 'USDC', symbol: 'USDC', logo: 'https://lh3.googleusercontent.com/aida-public/AB6AXuDuzgJMDLB780ZKeYzFsxpWrVkYF8r8iOeqUAxdPl4tbpTZZU3DcQjjvbAPPmXU1O1HfXwe8xEkktBVGII65_HknNEUIWXppcOZi0QL9SZDPBwrDoHLE0P6gKGC53E9WfCjQLOGnILm11uNDCda-7_gbtppUweRXF25exrsWK5omOMzYUO_KkFKDm0HQALIs2hnymfdvrbIzJ7-m2O7DB7UsjddNgQ2SfhTQEXw_es0YlbdCwPwffYrHSSjpbQ4tFoqju66qkiTiO0' }
  ];
  ...
  return (
    <div className="bg-background-light dark:bg-background-dark font-display relative mx-auto flex h-full min-h-screen w-full max-w-md flex-col">
      ...
      <div className="flex flex-1 flex-col px-4">
        {/* Wallet Selector */}
        <div className="py-4">
          <p className="mb-2 text-sm font-medium text-slate-500 dark:text-gray-400">Swap from</p>
          <div
        ...
        {/* Swap From Crypto Selector */}
        <div className="py-4">
          <p className="mb-2 text-sm font-medium text-slate-500 dark:text-gray-400">Swap from</p>
          <div
            onClick={() => setIsCryptoFromModalOpen(true)}
            className="flex cursor-pointer items-center gap-4 rounded-lg bg-slate-200/50 p-4 dark:bg-white/5"
          >
            <img
              alt={`${cryptos.find(c => c.symbol === selectedCryptoFrom)?.name} logo`}
              className="size-12 shrink-0 rounded-full"
              src={cryptos.find(c => c.symbol === selectedCryptoFrom)?.logo}
            />
            <div className="flex flex-col justify-center">
              <p className="text-base font-medium leading-normal text-slate-900 dark:text-white line-clamp-1">
                {cryptos.find(c => c.symbol === selectedCryptoFrom)?.name}
              </p>
              <p className="text-sm font-normal leading-normal text-slate-500 dark:text-gray-400 line-clamp-2">
                {selectedCryptoFrom}
              </p>
        ...
        {/* Swap To Crypto Selector */}
        <div className="py-4">
          <p className="mb-2 text-sm font-medium text-slate-500 dark:text-gray-400">Swap to</p>
          <div
            onClick={() => setIsCryptoToModalOpen(true)}
            className="flex cursor-pointer items-center gap-4 rounded-lg bg-slate-200/50 p-4 dark:bg-white/5"
          >
            <img
              alt={`${cryptos.find(c => c.symbol === selectedCryptoTo)?.name} logo`}
              className="size-12 shrink-0 rounded-full"
              src={cryptos.find(c => c.symbol === selectedCryptoTo)?.logo}
            />
            <div className="flex flex-col justify-center">
              <p className="text-base font-medium leading-normal text-slate-900 dark:text-white line-clamp-1">
                {cryptos.find(c => c.symbol === selectedCryptoTo)?.name}
              </p>
              <p className="text-sm font-normal leading-normal text-slate-500 dark:text-gray-400 line-clamp-2">
                {selectedCryptoTo}
              </p>
      ...
      {/* Crypto From Selection Modal */}
      {isCryptoFromModalOpen && (
        <div
          className="fixed inset-0 z-[60] flex flex-col justify-end bg-black/50"
          onClick={() => setIsCryptoFromModalOpen(false)}
        >
          <div
          ...
          >
            <div className="flex items-center justify-between p-4 pb-2 flex-shrink-0">
              ...
              <h3 className="text-lg font-bold text-slate-900 dark:text-white">Select Crypto to Swap From</h3>
              <button
                onClick={() => setIsCryptoFromModalOpen(false)}
              ...
              {cryptos.map((crypto) => (
                <label
                  ...
                  className={`flex cursor-pointer items-center gap-4 rounded-lg border border-solid p-4 has-[:checked]:border-primary has-[:checked]:bg-primary/10 border-slate-300 dark:border-slate-700 flex-shrink-0 ${
                    selectedCryptoFrom === crypto.symbol ? 'border-primary bg-primary/10' : ''
                  }`}
                >
                  <img
                  ...
                  <input
                    ...
                    name="crypto-from-modal"
                    type="radio"
                    checked={selectedCryptoFrom === crypto.symbol}
                    onChange={() => handleCryptoFromSelect(crypto.symbol)}
                  />
                </label>
              ))}
            </div>
            ...
            <div className="p-4 pt-2 pb-6 flex-shrink-0 border-t border-slate-200 dark:border-slate-800">
              ...
              >
                <span className="truncate">Select Crypto</span>
              </button>
            </div>
          </div>
        </div>
      ...
      {/* Crypto To Selection Modal */}
      {isCryptoToModalOpen && (
        <div
          className="fixed inset-0 z-[60] flex flex-col justify-end bg-black/50"
          onClick={() => setIsCryptoToModalOpen(false)}
        >
          <div
            className="flex flex-col rounded-t-xl bg-slate-100 dark:bg-slate-900 max-h-[80vh] flex-shrink-0"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between p-4 pb-2 flex-shrink-0">
              <div className="flex size-10 items-center justify-center"></div>
              <h3 className="text-lg font-bold text-slate-900 dark:text-white">Select Crypto to Swap To</h3>
              <button
                onClick={() => setIsCryptoToModalOpen(false)}
                className="flex size-10 items-center justify-center text-slate-500 dark:text-gray-400"
              >
            ...
            {/* Scrollable crypto list */}
            <div className="flex flex-col gap-3 p-4 overflow-y-auto flex-1 min-h-0">
              {cryptos.filter(crypto => crypto.symbol !== selectedCryptoFrom).map((crypto) => (
                <label
                  key={crypto.symbol}
                  className={`flex cursor-pointer items-center gap-4 rounded-lg border border-solid p-4 has-[:checked]:border-primary has-[:checked]:bg-primary/10 border-slate-300 dark:border-slate-700 flex-shrink-0 ${
                    selectedCryptoTo === crypto.symbol ? 'border-primary bg-primary/10' : ''
                  }`}
                >
                  <img
                    alt={`${crypto.name} logo`}
                  ...
                  <input
                    className="form-radio radio-custom ml-auto h-5 w-5 cursor-pointer border-2 bg-transparent text-transparent focus:outline-none focus:ring-0 focus:ring-offset-0 border-slate-300 dark:border-slate-700 checked:border-primary"
                    name="crypto-to-modal"
                    type="radio"
                    checked={selectedCryptoTo === crypto.symbol}
                    onChange={() => handleCryptoToSelect(crypto.symbol)}
                  />
                </label>
              ))}
            </div>
...
export default SelectAssetSwap;
