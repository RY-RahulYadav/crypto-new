  ...
  return (
    <div className="bg-background-light dark:bg-background-dark font-display relative mx-auto flex h-full min-h-screen w-full max-w-md flex-col">
      ...
      <div className="flex flex-1 flex-col px-4">
        {/* Wallet Selector */}
        <div className="py-4">
          <p className="mb-2 text-sm font-medium text-slate-500 dark:text-gray-400">Sell from</p>
          <div
        ...
        <div className="flex flex-col gap-3 py-3">
          {cryptos.map((crypto) => (
            <label
              key={crypto.symbol}
              className={`flex cursor-pointer items-center gap-4 rounded-lg border border-solid p-4 has-[:checked]:border-primary has-[:checked]:bg-primary/10 border-slate-300 dark:border-slate-700 ${
                selectedCrypto === crypto.symbol ? 'border-primary bg-primary/10' : ''
              }`}
            >
              <img
              ...
              <div className="flex grow flex-col">
                <p className="text-base font-medium leading-normal text-slate-900 dark:text-white">{crypto.name}</p>
                <p className="text-sm font-normal leading-normal text-slate-500 dark:text-gray-400">{crypto.symbol}</p>
              </div>
              <input
...
export default SelectAssetSell;
